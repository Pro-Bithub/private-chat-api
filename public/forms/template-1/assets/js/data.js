...
fetch(AssetsUrl)
  ...
  .then((data) => {
    console.log(data);
    if (data.data.hasOwnProperty("lang")) if (data.data.lang == 2) lang = "fr";
        ...
        $.ajax({
          ...
          success: function (responseathu) {
            ...
            if (responseathu.success == "true") {
              ...
              var indexOfMatch = defaultmessegelogin_faild.findIndex(
              ...
              if (indexOfMatch !== -1) {
                const index = String(defaultmessegelogin_faild[indexOfMatch])
                  .toLowerCase()
                  .replace(/\s+/g, "");
                console.log(index);
