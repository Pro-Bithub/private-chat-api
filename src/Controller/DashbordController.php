...
class DashbordController extends AbstractController
{
    ...
    public function getDashbordData(Request $request,EntityManagerInterface $entityManagerInterface): Response
    {
       ...
       //SalesByCountry
        ...
        'SELECT c.country, 
        COUNT(CASE 
            WHEN MONTH(s.date_creation) = MONTH(CURRENT_DATE) AND YEAR(s.date_creation) = YEAR(CURRENT_DATE)
            THEN c.country
            ELSE NULL
      ...
      AND YEAR(s.date_creation) = YEAR(NOW())
       ...
       'SELECT  p.name, (SUM(
        ...
        //Conversion Rate
            ...
            ROUND( (COUNT(DISTINCT c.id) / (SELECT COUNT(*) FROM contacts)) * 100,1) AS percentage_contacts_with_sales
           FROM sales AS s
           INNER JOIN contacts AS c ON c.id = s.contact_id
           INNER JOIN plans AS p ON p.id = s.plan_id
           WHERE p.account_id = :id
